...
async function addQuizToFirstCourse() {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.MONGODB_URL)
    console.log('MongoDB connected')

    // Get the first course to use its ID
    const Course = mongoose.model('Course', new mongoose.Schema())
    const courses = await mongoose.connection.collection('courses').find({}).limit(1).toArray()
    ...
    await quiz.save()
    console.log('Quiz added successfully to course:', courseId)
    process.exit(0)
  } catch (error) {
